...
const OptionList: React.FC<Props> = ({ question, answer, onOptionClicked, showResult }) => {
  const getIcon = (option: QuestionOption): React.ReactNode => {
    if (!showResult) return <Icon icon={option.id === answer ? radioButtonOn : radioButtonOff} />;

    //The correct answer (chosen)
    if (option.id === answer && answer === question.answer) return <Icon icon={checkmarkCircle} />;
