...
describe('state > study > navigation > selectors', () => {
    ...
    'nav.introduction': [
      {
        ...
        description: 'K53 Ninja contains everything you need to know to pass your Learner&#39;s Licence test',
      },
      {
        imageName: '',
  ...
  it('currentNavigationChildrenSelector', () => {
    const actual = selectors.currentNavigationChildrenSelector.resultFunc(navigationData, 'nav.roadSignals');

    expect(actual).toEqual(['nav.roadSignals.regulatorySignals', 'nav.roadSignals.warningSignals']);
  });

  it('currentNavigationParentSelector', () => {
    const actual = selectors.currentNavigationParentSelector.resultFunc('nav.signs.guidance.freewayDirectionSigns');
