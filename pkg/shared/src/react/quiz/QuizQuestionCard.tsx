...
const QuizQuestionCard: React.FC<Props> = ({
  ...
  answer,
  showResult,
  imageAssetBaseUrl = "assets/images",
  onOptionClicked,
}) => {
  ...
  const imageUrl = (image: string): string => `${imageAssetBaseUrl.replace(/\/$/, "")}/${image}`;
  ...
  return (
    <Card>
      ...
      {(question.image || question.image2) && (
        <ImageFrame>
          {question.image && <QuestionImage alt="" src={imageUrl(question.image)} />}
          {question.image2 && <QuestionImage alt="" src={imageUrl(question.image2)} />}
        </ImageFrame>
      )}
      <Options>
...
const ImageFrame = styled.div`
  ...
  border-radius: 20px;
  background: var(--app-question-image-background, var(--app-quiz-background));
`;
