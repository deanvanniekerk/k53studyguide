...
const rootSelector: Selector<RootState, LogState> = (state: RootState): LogState => state.quiz.log;
...
export const quizLevelSelector: OutputSelector<RootState, number, (current: number) => number> = createSelector(
...
export const quizCurrentExperiencePercentSelector: OutputSelector<
  RootState,
  number,
  (current: number, level: number) => number
> = createSelector(quesionsSuccesfullyAnsweredSelector, quizLevelSelector, (current, level) => {
